...
public class Heart : MonoBehaviour
{
   ...
   void Start()
   {
       ...
       for (int i =0; i < numSphere; i++){
           ...
           endPosition[i] = new Vector3(
               ...
               10f);
           */
               //r * Mathf.Sin(i * 2 * Mathf.PI / numSphere),
               //r * Mathf.Cos(i * 2 * Mathf.PI / numSphere));
       }
   ...
   void Update()
   {
       ...
       for (int i =0; i < numSphere; i++){
           ...
           //update end pos, alters the movement
           // Circular end position
           float sin = Mathf.Sin(i * 2 * Mathf.PI / numSphere);
           float cos = Mathf.Cos(i * 2 * Mathf.PI / numSphere);
           ...
           hueControl = Mathf.Cos(time) * 0.1f * 0.1f; //allow color to oscillate over time
           Color color = Color.HSVToRGB(Mathf.Abs(hue * Mathf.Sin(time)), Mathf.Cos(time), 2f + Mathf.Cos(time)); // Full saturation and brightness
           //Color color = Color.HSVToRGB(hueControl, 1f, 1f); //change first val to change hues
